...
class Chatbot:
    ...
    def __init__(self, lang='en',RL = True, user_id = 'name',age = 64, instructions = True):
        ...
        if self.model_type == True:
            ...
            self.model.load_state_dict(torch.load("seq2seq/models/checkpoint2_50", map_location=torch.device(self.device)))
    ...
    def define_voice(self,lang='en'):
        if lang == 'es':
            self.voice = 0#14  # 14: spanish male
        else:
            self.voice = 1#0  # 0:english male

    def reset_chatbot(self, lang='en'):
        self.chat_history_ids = self.tokenizer.encode(self.tokenizer.bos_token, return_tensors='pt').to(self.device)
    ...
    def speak(self, text):
        speech.text_to_speech(text, speed = 180, vol=1.0, voice=self.voice)
...
chatbot = Chatbot(lang='es', RL=True, user_id='RL-Enric', age = 24, instructions=False)
chatbot.run_chat()
